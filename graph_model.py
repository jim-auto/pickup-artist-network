  ...
  <script>
      ...
      style: [
        {
          ...
          style: {
            ...
            "text-outline-color": graphLabelStroke,
            "text-valign": "bottom",
            "text-margin-y": 4,
            "overlay-opacity": 0,
        ...
        },
        {
          ...
          style: {
            ...
            height: 22,
            padding: "8px",
            "background-color": prefersDarkGraph ? "#141c26" : "#ffffff",
    ...
    function graphColor(value) {
      if (value && typeof value === "object") {
        return value.color || graphEdgeColor;
      }
      return value || graphEdgeColor;
    }
    ...
    function graphContentNodes() {
      return cy.nodes().filter((node) => node.data("kind") !== "cluster-label");
    }
    ...
    const network = {
      ...
      getViewPosition() {
        return graphViewCenter();
      },
      moveTo({ position, scale, animation } = {}) {
        const zoom = scale == null ? cy.zoom() : scale;
        const pos = position || graphViewCenter();
        const pan = {
          x: cy.width() / 2 - zoom * pos.x,
          y: cy.height() / 2 - zoom * pos.y
        };
        if (animation && animation.duration) {
          cy.animate({ zoom, pan, duration: animation.duration, easing: "ease-in-out-cubic" });
          return;
        }
        syncingCamera = true;
      ...
      getSelectedNodes() {
        return graphContentNodes().filter(":selected").map((node) => node.id());
      },
      getNodeAt(pointer) {
        const hit = graphContentNodes().filter((node) => {
          const box = node.renderedBoundingBox({ includeLabels: false, includeOverlays: false });
          return pointer.x >= box.x1 && pointer.x <= box.x2 && pointer.y >= box.y1 && pointer.y <= box.y2;
        });
        return hit.length ? hit[0].id() : undefined;
      },
      canvasToDOM(pos) {
        const pan = cy.pan();
        const zoom = cy.zoom();
      ...
      focus(nodeId, options = {}) {
        ...
        const scale = options.scale == null ? Math.max(cy.zoom(), 1.05) : options.scale;
        this.moveTo({
      ...
      openCluster() {},
      get body() {
        return { nodeIndices: graphContentNodes().map((node) => node.id()) };
      },
      interactionHandler: { options: { zoomView: true, dragView: true } }
    };
    ...
    cy.on("tap", "node", (event) => {
      if (event.target.hasClass("cluster-label")) {
        return;
      }
    ...
    cy.on("mouseover", "node, edge", (event) => {
      networkEl.title = event.target.data("title") || "";
    });
    ...
    function focusNode(nodeId) {
      ...
      if (currentVisibleNodes.some((node) => node.id === nodeId)) {
        network.focus(nodeId, {
          scale: Math.max(network.getScale(), 1.05),
          animation: {
            duration: 300,
    ...
    function updateNetworkEmphasis(selectedId = null) {
      ...
      graphContentNodes().forEach((ele) => {
        const nodeId = ele.id();
        ele.data(
          "label",
          selectedId && neighborIds.has(nodeId)
            ? (rawNodeById.get(nodeId)?.name || ele.data("label"))
            : getNodeLabel(rawNodeById.get(nodeId) || { id: nodeId, name: ele.data("label") }, currentVisibleNodes, currentSearchTerm)
        );
      });
      if (!selectedId) {
        return;
      }
      currentVisibleEdges.forEach((edge, index) => {
        if (edge.source === selectedId || edge.target === selectedId) {
          cy.getElementById(`${edge.source}-${edge.target}-${edge.type}-${index}`).addClass("emphasized");
        }
      });
    }
    ...
    function applyFilters() {
      ...
      const showArrows = drawableEdges.length <= 1200;
      const elements = [
        ...visibleNodes.map((node) => {
          const value = getNodeVisualValue(node, visibleNodes);
          const hasIcon = shouldShowNodeIcon(node, visibleNodes);
          return {
            group: "nodes",
            data: {
              id: node.id,
              ...
              kind: "account",
              size: Math.max(8, value * 1.35),
              color: nodeColors[node.type] || "#64748b",
              borderWidth: (node.follower_count || 0) > 0 ? 2 : 1,
              borderColor: (node.follower_count || 0) > 0 ? "#ffffff" : "#d8e2f0",
              image: hasIcon ? node.icon_url : undefined,
              title: [
        ...
        ...clusterLabels.map((item) => ({
          ...
          data: {
            ...
            label: item.label,
            kind: "cluster-label",
            labelWidth: Math.max(56, String(item.label || "").length * 8.2),
            size: 22,
            color: prefersDarkGraph ? "#141c26" : "#ffffff",
            borderWidth: 0,
        ...
        ...drawableEdges.map((edge, index) => ({
          ...
          data: {
            ...
            source: edge.source,
            target: edge.target,
            label: drawableEdges.length <= 260 ? formatEdgeType(edge.type) : "",
            color: graphColor(
              isAssistiveEdge(edge)
                ? assistiveEdgeColor(edge, drawableEdges)
                : getEdgeColor(drawableEdges)
            ),
            width: isWeakAssistiveEdge(edge) ? 0.4 : (drawableEdges.length > 1200 ? 0.45 : 1),
            lineStyle: isAssistiveEdge(edge) && drawableEdges.length <= 1200
              ? (isWeakAssistiveEdge(edge) ? "dotted" : "dashed")
              : "solid",
            arrow: showArrows ? "triangle" : "none",
      ...
      if (!(allowAutoFit && !selectedNodeId && !term) && Number.isFinite(savedCamera.zoom)) {
        syncingCamera = true;
        cy.viewport(savedCamera);
        syncingCamera = false;
      }
