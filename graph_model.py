...
<body>
  ...
  <main>
    ...
    <section class="panel social-dashboard">
      <div class="network-head">
        <div>
          <h2>Xアカウント分析</h2>
          <p class="muted">フォロワー数、アイコン、接続密度から、見るべき人物と追加調査候補を出します。</p>
        </div>
        <div id="dashboard-summary" class="view-summary"></div>
      </div>
      <div id="social-dashboard-grid" class="dashboard-grid"></div>
    </section>
  ...
  <script>
    ...
    function influenceScore(node) {
      const followers = node.follower_count || 0;
      const degree = nodeDegreeById.get(node.id) || 0;
      const follows = followDegreeById.get(node.id) || 0;
      return Math.round(Math.log10(followers + 10) * 100 + degree * 3 + follows * 18);
    }

    function renderRankRows(nodes, metaForNode) {
      if (!nodes.length) {
      ...
      return `
        <div class="rank-list">
          ${nodes.map((node, index) => `
            <button type="button" class="rank-row" data-node-id="${escapeHtml(node.id)}">
              ${formatNodeAvatar(node, "avatar-thumb")}
              <span class="node-name-text">
                <strong>#${escapeHtml(index + 1)} ${escapeHtml(node.name)}</strong>
                <span class="muted">${escapeHtml(metaForNode(node))}</span>
              </span>
            </button>
          `).join("")}
        </div>
    ...
    function renderSocialDashboard(visibleNodes, visibleEdges) {
      const summary = document.getElementById("dashboard-summary");
      const grid = document.getElementById("social-dashboard-grid");
      if (!summary || !grid) {
        return;
      }
      const visiblePersonIds = new Set(visibleNodes.filter((node) => node.type === "person").map((node) => node.id));
      const visiblePeople = rawGraph.nodes.filter((node) => node.type === "person" && visiblePersonIds.has(node.id));
      const allPeople = rawGraph.nodes.filter((node) => node.type === "person");
      const peopleWithFollowers = allPeople.filter((node) => (node.follower_count || 0) > 0);
      const visibleWithFollowers = visiblePeople.filter((node) => (node.follower_count || 0) > 0);
      const visibleRelevant = visiblePeople.filter((node) => isNetworkRelevantPerson(node));
      const coverage = allPeople.length ? Math.round((peopleWithFollowers.length / allPeople.length) * 100) : 0;
      const visibleCoverage = visiblePeople.length ? Math.round((visibleWithFollowers.length / visiblePeople.length) * 100) : 0;
      const personDegrees = visiblePersonDegreeById(visibleEdges);
      const topFollowers = visibleRelevant
        .filter((node) => hasRealProfileIcon(node) && (node.follower_count || 0) > 0)
        .sort((left, right) =>
          (right.follower_count || 0) - (left.follower_count || 0) ||
          influenceScore(right) - influenceScore(left) ||
          left.name.localeCompare(right.name, "ja")
        )
        .slice(0, 6);
      const topInfluence = visibleRelevant
        .filter((node) => hasRealProfileIcon(node) && (node.follower_count || 0) > 0)
        .sort((left, right) =>
          influenceScore(right) - influenceScore(left) ||
          (right.follower_count || 0) - (left.follower_count || 0) ||
        ...
        .slice(0, 6);
      const sparseHighFollowers = visibleRelevant
        .filter((node) => (node.follower_count || 0) >= 3000 && (personDegrees.get(node.id) || 0) < 8)
        .sort((left, right) =>
          (right.follower_count || 0) - (left.follower_count || 0) ||
        ...
        .slice(0, 6);
      const missingFollowers = visibleRelevant
        .filter((node) => (node.follower_count || 0) <= 0)
        .sort((left, right) =>
          Number(hasRealProfileIcon(right)) - Number(hasRealProfileIcon(left)) ||
          (nodeDegreeById.get(right.id) || 0) - (nodeDegreeById.get(left.id) || 0) ||
          left.name.localeCompare(right.name, "ja")
        )
        .slice(0, 6);
      ...
      summary.innerHTML = `
        <span class="tag">取得率 ${escapeHtml(coverage)}%</span>
        <span class="tag">表示内 ${escapeHtml(visibleCoverage)}%</span>
        <span class="tag">未取得 ${escapeHtml(allPeople.length - peopleWithFollowers.length)} 人</span>
      `;
      grid.innerHTML = `
        <section class="dashboard-card">
          <h3>フォロワー取得状況</h3>
          <span class="metric-value">${escapeHtml(formatNumber(peopleWithFollowers.length))} / ${escapeHtml(formatNumber(allPeople.length))}</span>
          <span class="metric-sub">表示中: ${escapeHtml(formatNumber(visibleWithFollowers.length))} / ${escapeHtml(formatNumber(visiblePeople.length))}</span>
        </section>
        <section class="dashboard-card">
          <h3>フォロワー上位</h3>
          ${renderRankRows(topFollowers, (node) => `X followers: ${formatNumber(node.follower_count || 0)}`)}
        </section>
        <section class="dashboard-card">
          <h3>影響力スコア</h3>
          ${renderRankRows(topInfluence, (node) => `score ${formatNumber(influenceScore(node))} / follow ${formatNumber(followDegreeById.get(node.id) || 0)}`)}
        </section>
        <section class="dashboard-card">
          <h3>接続精査候補</h3>
          ${renderRankRows(sparseHighFollowers, (node) => `人物間 ${formatNumber(personDegrees.get(node.id) || 0)} / X followers ${formatNumber(node.follower_count || 0)}`)}
        </section>
        <section class="dashboard-card">
          <h3>フォロワー未取得</h3>
          ${renderRankRows(missingFollowers, (node) => `接続 ${formatNumber(nodeDegreeById.get(node.id) || 0)} / アイコン ${hasRealProfileIcon(node) ? "あり" : "なし"}`)}
        </section>
      `;
    }
    ...
    function applyFilters() {
      ...
      renderSocialDashboard(visibleNodes, visibleEdges);
      renderVisibleTables();
    ...
    document.getElementById("social-dashboard-grid").addEventListener("click", (event) => {
      const button = event.target.closest("[data-node-id]");
      if (!button) {
        return;
      }
