...
class ScraperSourceSnapshotTests(unittest.TestCase):
    ...
    def test_build_growth_targets_payload_counts_only_real_seed_entities(self) -> None:
        ...
        self.assertEqual(payload["headline"]["target"], 20)
    ...
    def test_format_growth_targets_output_lists_headline_and_ranges(self) -> None:
        output = format_growth_targets_output(
            {
                "headline": {"label": "Real person target", "current": 3, "target": 20},
                "phases": [{"label": "Phase 1", "real_person_target": 10}],
                "types": [
                    {"type": "person", "current": 3, "target_min": 20, "target_max": 20},
                    {"type": "community", "current": 2, "target_min": 8, "target_max": 12},
                ],
            }
        )

        self.assertIn("[OK] Real person target: 3 / 20", output)
        self.assertIn("- Phase 1: real person target 10", output)
        self.assertIn("- person: 3 / 20", output)
